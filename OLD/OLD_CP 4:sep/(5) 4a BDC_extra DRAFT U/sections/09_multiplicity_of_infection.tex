...
\label{eq:Kk}
\end{align}
The expected release flux --- the quantity on which the population-level
renewal model of Chapter~4b will be built --- is therefore
\begin{equation}
g_k(t)=\delta K_k(t)
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\textwidth]{figures/N4a_7_moi_flux_ratios.pdf}
\caption{Release-flux ratios $g_k(t)/(k g_1(t))$ for $k=2,3,4$ at
$(\lambda,\mu,\delta)=(1,0.2,0.05)$. Values above $1$ mark superlinear
release relative to $k$ independent single-founder cells. All three ratios
are superlinear early, but their crossings and subsequent decay show that
the enhancement is age-dependent rather than a pointwise ordering valid for
all $t$.}
\label{fig:N4a_7}
\end{figure}
...
is $\mathrm{Geom}_0(s)$ with mean $\rho/s=\lambda/\delta$ whatever the
founding load, and the total release is simply $k$ founders plus that many
births. For example, with $\lambda=1$, $\delta=0.1$:
...
release. At $(\lambda,\mu,\delta)=(1,0.2,0.05)$: $V_\infty\approx13.99$, so a
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\textwidth]{figures/N4a_8_yield_subextensive.pdf}
\caption{Unconditional mean yield $V_\infty^{(k)}$ against founding
multiplicity $k$, compared with the naive independent-cell line
$kV_\infty$, for $(\lambda,\mu,\delta)=(1,0.2,0.05)$. The true yield is
sub-extensive because all founders share one cell and hence one catastrophe;
for $k=2$, $V_\infty^{(2)}=17.43$, rather than
$2V_\infty=27.97$.}
\label{fig:N4a_8}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/F4a_6_moi.pdf}
\caption{Multiplicity of infection at $(\lambda,\mu,\delta)=(1,0.2,0.05)$.
(a)~Release fluxes $g_k(t)=\delta K_k(t)$ for $k=1,\ldots,4$: early ages are ordered superlinearly in founding load, while late fluxes reverse as multi-founder lineages fix faster.
(b)~Conditional mean burst $V_\infty^{(k)}/(1-b^k)$ increases with $k$; the dashed line is the $k=1$ value $a/(a-1)$.}
\label{fig:F4a_6_moi}
\end{figure}

% \figureflag{F4a.6}{Multiplicity of infection}{%
% \textbf{Purpose:} display the two MOI effects of this section --- the
% superlinear release flux in founding multiplicity, and the increasing
% conditional mean burst --- in one figure. \textbf{Type:} Python analytic
% plot, two panels (or inset). \textbf{Content:} panel (a): the release fluxes
% $g_k(t)=\delta K_k(t)$ against age $t\in[0,15]$ for $k=1,2,3,4$, with
% $(\lambda,\mu,\delta)=(1,0.2,0.05)$, where
% $K_k=kKI^{k-1}+k(k-1)J^2I^{k-2}$ from \eqref{eq:Kk} and $I,J,K$ are the
% closed forms of Chapter~3 (\Eqref{eq:IDIhat}--\eqref{eq:K}); annotate the
% early superlinear ordering (and the late reversal). Panel (b): the conditional mean burst
% $V_\infty^{(k)}/(1-b^k)$ against $k=1,\ldots,6$ from \eqref{eq:Vk}, with
% the horizontal dashed line at $V_\infty/(1-b)=a/(a-1)$ (the $k=1$ value) to
% show the increase. \textbf{Axes:} panel (a): age $a$ (horizontal; label the
% axis ``age'' or ``time since infection'' to avoid collision with the root
% $a$ --- use ``age'' on the axis), release flux $g_k$ (vertical); panel (b):
% founders $k$ (horizontal), conditional mean burst (vertical); legend entries
% $k=1,2,3,4$. \textbf{Style:} unified Python style --- default matplotlib
% with \texttt{tab:} colours, grid alpha 0.25, \texttt{tight\_layout}, PDF
% output, fonts matching the chapter. \textbf{Data source:} analytic formulas
% \eqref{eq:Kk}, \eqref{eq:Vk}; no simulation. \textbf{Placement:}
% \Secref{sec:moi}, at the end of the section.}
