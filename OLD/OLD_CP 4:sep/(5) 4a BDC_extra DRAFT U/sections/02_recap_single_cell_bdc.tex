...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/F4a_1_joint_process.pdf}
\caption{A representative path of the joint process $(X_t,W_t)$.  The
intracellular load follows a birth--death path until rupture at time $\tau$,
when $X_t$ enters the absorbing state $H$.  At the same instant, the released
count jumps from zero to the pre-rupture load $X_{\tau^-}$ and remains there.}
\label{fig:F4a_1_joint_process}
\end{figure}
