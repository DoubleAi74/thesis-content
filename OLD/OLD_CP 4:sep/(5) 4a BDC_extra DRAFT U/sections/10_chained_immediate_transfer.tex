...
$\lambda n$, catastrophe $\delta n$); on rupture, however, the released load
...
$\mathrm{Exp}\lrb{(\lambda+\delta)n}$. Consequently the sequence of event
...
\begin{remark}[The coefficient, and a near miss]
The coefficient in \eqref{eq:rklaw} is easy to miscount. The superficially
similar expression $\frac{1}{k!}\frac{(n+k-1)!}{(n-1)!}
=\binom{n+k-1}{k}$ is off by one shift, and is already contradicted by the
$r_2$ and $r_3$ cases displayed above, which \eqref{eq:rklaw} reproduces
exactly. The correct coefficient is $\binom{n+k-2}{k-1}$, and it has a
direct reading: it counts the ways of distributing the $n-1$ births
accumulated along the chain among its $k$ cells, each cell receiving any
non-negative number --- the stars-and-bars count for \eqref{eq:rkdecomp}.
\end{remark}

Further consequences, all verified numerically (see
\S\ref{sec:chained-verification}):
\begin{align}
\mean{r_k}&=1+k\,\frac{\rho}{s}=1+k\,\frac{\lambda}{\delta},
&
...
expected burst's worth of births, $\lambda/\delta$, to whatever it inherited.
...
=\sum_{g\ge0}s\rho^g\prod_{j=k}^{k+g}\frac{(\lambda+\delta)j}{(\lambda+\delta)j+u}.
\label{eq:LTk}
\end{equation}
Writing $u'=u/(\lambda+\delta)$ and unrolling the product with Gamma
functions, the series sums to a hypergeometric form --- a theme that recurs
in Chapter~4b, where the effective parameters of the population model are
likewise expressed through hypergeometric Laplace transforms:
\begin{equation}
...
\begin{equation}
\mean{T(k)}=\sum_{m\ge0}\frac{\rho^m}{(\lambda+\delta)(k+m)},
\label{eq:meanT}
\end{equation}
...
For $(\lambda,\delta)=(1,1)$: $\mean{T(1..4)}\approx0.693,0.386,0.273,0.212$
...
\qquad
P_1(m,u)=\prod_{j=1}^{m}\frac{(\lambda+\delta)j}{(\lambda+\delta)j+u}.
\end{equation}
...
histograms); and the monotone interval means. \textbf{28 checks, all
passing.}

What remains open, and is stated plainly: the joint laws of $(t_k,r_k)$ for
general $\mu>0$ (a coefficient machinery with ${}_2F_1$ antiderivatives is
the route, and the single-cell ingredients are in
...
% \figureflag{F4a.7}{Chained transfer}{%
% \textbf{Purpose:} show the section's two headline structures --- the
% negative-binomial rupture-size laws and the decreasing inter-rupture
% intervals --- in one figure. \textbf{Type:} Python simulation + analytic
% overlay, two panels. \textbf{Content:} panel (a): rupture-size pmfs
% $\pr{r_k=n}$ for $k=1,2,3,4$ with $(\lambda,\delta)=(1,1)$ (so
% $s=\rho=1/2$): histogram bars from exact simulation of $10^6$ chains (the
% event-type decomposition makes sampling direct), with the closed forms
% $\binom{n+k-2}{k-1}s^k\rho^{n-1}$ from \eqref{eq:rklaw} overlaid as
% connected curves. Panel (b): mean inter-rupture intervals
% $\mean{T(k)}=\sum_{m\ge0}\rho^m/((\lambda+\delta)(k+m))$ from
% \eqref{eq:meanT} against $k=1,\ldots,6$, plotted as a decreasing curve,
% with simulated chain-interval means (random founders) overlaid as points to
% show they decrease faster. Adapt the exact-sampling machinery of the
% verification suite (do not modify the suite itself; write a chapter-local
% script). \textbf{Axes:} panel (a): rupture size $n$ (horizontal),
% probability (vertical), legend $k=1..4$; panel (b): $k$ (horizontal), mean
% interval (vertical). \textbf{Style:} unified Python style --- default
% matplotlib with \texttt{tab:} colours, grid alpha 0.25,
% \texttt{tight\_layout}, PDF output, fonts matching the chapter.
% \textbf{Data source:} closed forms \eqref{eq:rklaw}, \eqref{eq:meanT};
% simulation via the event-type decomposition ($G_i$ i.i.d.\
% $\mathrm{Geom}_0(s)$; exponential holding times of rate
% $(\lambda+\delta)n$). \textbf{Placement:} \Secref{sec:chained}, after the
% verification record.}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/F4a_7_chained_transfer.pdf}
\caption{Chained immediate transfer with $(\lambda,\delta)=(1,1)$ and
$s=\rho=1/2$. (a) Simulated rupture-size probabilities (bars) from
$10^6$ chain realisations with seed 42, together with the exact
negative-binomial laws (connected curves) for $k=1,\ldots,4$. (b) The
analytic mean interval for a fixed founding load $k$ and the simulated mean
interval at chain position $k$; both decrease over $k=1,\ldots,6$.}
\label{fig:F4a_7_chained_transfer}
\end{figure}
