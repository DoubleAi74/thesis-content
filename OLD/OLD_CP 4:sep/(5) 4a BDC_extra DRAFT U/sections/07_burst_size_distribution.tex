...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/F4a_4_burst_time_density.pdf}
...
$1-b$ and the remaining mass $b=0.188$ lies at $\tau=\infty$; for
$(1,0,0.1)$, $b=0$ and the defective and conditional densities coincide.}
\label{fig:F4a_4_burst_time_density}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/F4a_5_burst_size_late.pdf}
\caption{Burst-size and late-burst laws for
$(\lambda,\mu,\delta)=(1,0.2,0.05)$. (a) The unconditional law
$(\delta/\lambda)a^{-k}$, including the no-burst mass $b$ at $k=0$, and
the law $(a-1)a^{-k}$ conditioned on bursting. (b) The size-biased mean
$\mean{\Kb\mid\tau=t}$ rises towards $(a+1)/(a-1)=33.46$.}
\label{fig:F4a_5_burst_size_late}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{figures/N4a_5_joint_tau_K.pdf}
\caption{Joint density (with respect to time and counting measure) of burst
time $\tau$ and burst size $\Kb$ for
$(\lambda,\mu,\delta)=(1,0,0.1)$. The analytic overlay is the conditional
mean $\mean{\Kb\mid\tau=t}=K(t)/J(t)$, which rises from $1$ towards $21$.
The upward tilt makes the geometric content of size-biasing explicit: later
bursts preferentially occur from larger loads.}
\label{fig:N4a_5}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/N4a_3_burst_vs_budding_pmf.pdf}
\caption{Release-count laws for the BDC process at
$(\lambda,\mu,\delta)=(1,0.2,0.05)$ and a budding comparator. The budding
mean $p/d_{\Icell}$ is matched to the BDC conditional mean
$V_\infty/(1-b)=17.23$. The BDC law conditioned on bursting is geometric on
$\{1,2,\ldots\}$, whereas the budding law is geometric on
$\{0,1,2,\ldots\}$ and retains probability $0.055$ of zero release. Thus
matching the means neither matches the support nor the tails.}
\label{fig:N4a_3}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/F4a_8_tau_given_k.pdf}
\caption{Conditional mean rupture time given burst size $n$ when $\mu=0$,
with $\lambda=1$, $\delta=0.1$, and hence $\theta=1.1$. Exact harmonic
means $\theta^{-1}H_n$ are shown with the large-$n$ approximation
$\theta^{-1}(\log n+\gamma)$ and the linear budding comparator
$(n+1)/(d_{\Icell}+p)$ at $d_{\Icell}+p=8$.}
\label{fig:F4a_8_tau_given_k}
\end{figure}
