...
PDF_PATH = CHAPTER_DIR / "figures" / "N4a_3_burst_vs_budding_pmf.pdf"
PNG_PATH = WORKDIR / "preview.png"
BDC_COLOUR = "tab:orange"
BUD_COLOUR = "tab:blue"
...
def make_figure() -> None:
    ...
    fig, (ax_pmf, ax_tail) = plt.subplots(1, 2, figsize=(9.8, 4.25))
    ...
    ax_tail.plot(
        ...
        markevery=7,
        linewidth=2.1,
        label=r"BDC $\Pr(\mathcal{K}\geq k)$",
    )
    ax_tail.plot(
        k_tail,
        bud_tail,
        ...
        markevery=7,
        linewidth=2.0,
        label=r"budding $\Pr(\mathcal{K}_{\rm bud}\geq k)$",
    )
    ax_tail.scatter(
        [tail_crossing],
        [crossing_height],
    ...
    ax_tail.annotate(
        rf"tails cross near $k={tail_crossing:.1f}$",
        xy=(tail_crossing, crossing_height),
        xytext=(31.0, 0.57),
        arrowprops={"arrowstyle": "->", "color": "#333333", "linewidth": 0.85},
        fontsize=9,
        ha="center",
    ...
    ax_tail.text(
        0.97,
        0.94,
        rf"matched mean $={matched_mean:.2f}$ for both",
        transform=ax_tail.transAxes,
        ha="right",
        va="top",
        fontsize=9,
    )
    ax_tail.set_xlim(1.0, 65.0)
    ax_tail.set_ylim(0.0, 1.03)
    ax_tail.set_xlabel(r"threshold $k$")
    ax_tail.set_ylabel("tail probability")
    ax_tail.set_title(
        r"(b) Equal means do not match the tails",
        loc="left",
        fontweight="bold",
    )
    ax_tail.legend(loc="upper right", bbox_to_anchor=(1.0, 0.84))
