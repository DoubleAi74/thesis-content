...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/F3_1_process_diagram.pdf}
\caption{Transition diagram of the birth--death--catastrophe process. From a
non-absorbing state $n$, births occur at rate $\lambda n$, deaths at rate
$\mu n$, and catastrophe moves the process to $H$ at rate $\delta n$.
States $0$ and $H$ are absorbing.}
\label{fig:F3_1_process_diagram}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/F3_6_gillespie_X.pdf}
\caption{One hundred Gillespie realisations of $X_t$, re-rendered on
$0\leq t\leq20$. The terminal step to the plotting baseline marks entry into
the absorbing catastrophe state $H$, rather than the count state $0$.
$\lambda=1$, $\mu=0$, $\delta=0.1$, and $X_0=1$.}
\label{fig:F3_6_gillespie_X}
\end{figure}

A second ensemble, now resolved by terminal fate, exposes the structure hidden
by an undifferentiated bundle of sample paths. In particular, the ensemble
mean need not resemble any one of the realisations from which it is formed.

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/N3_5_fate_coloured_paths.pdf}
\caption{Terminal-fate decomposition of 180 exact Gillespie realisations of
$X_t$. Blue-grey paths ending in crosses reach internal extinction at $0$;
red paths ending in filled circles enter the separate catastrophe state $H$.
The bold curve is the exact mean $J(t)=\mathbb{E}[X_t]$. Because both absorbed
classes subsequently contribute zero, this mean mixes structurally different
histories rather than describing a typical path. Here $\lambda=1$, $\mu=0.2$,
$\delta=0.05$, $X_0=1$, and the fixed simulation seed is 3505.}
\label{fig:N3_5}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/F3_2_killing_vs_catastrophe.pdf}
\caption{Killing and catastrophe conventions for rupture. Under the killing
convention, a transition at rate $\delta n$ returns the process from state
$n$ to the absorbing state $0$. Under the catastrophe convention used here,
the same transition ends at the separate absorbing state $H$; the birth and
death rates remain $\lambda n$ and $\mu n$, respectively.}
\label{fig:F3_2_killing_vs_catastrophe}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/F3_6_gillespie_W.pdf}
\caption{One hundred corresponding Gillespie realisations of $W_t$,
re-rendered on $0\leq t\leq20$. Each path remains at zero until rupture, then
holds at the released count $X_{\tau^-}$; the arrow marks one representative
burst. $\lambda=1$, $\mu=0$, $\delta=0.1$, and $X_0=1$.}
\label{fig:F3_6_gillespie_W}
\end{figure}
