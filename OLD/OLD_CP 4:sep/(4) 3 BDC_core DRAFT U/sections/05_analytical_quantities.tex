...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/F3_3_fixation.pdf}
\caption{Non-catastrophe probability $I(t)$, internal-extinction probability
$D(t)$, and non-fixation probability $\Ifix(t)=I(t)-D(t)$. For
$\lambda=1$, $\mu=0.2$, and $\delta=0.05$, both $I(t)$ and $D(t)$ approach
$b$, while $\Ifix(t)$ tends to zero with initial slope
$\Ifix'(0)=-(\mu+\delta)=-0.25$.}
\label{fig:F3_3_fixation}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/N3_3_multi_founder_fixation.pdf}
\caption{Multi-founder non-fixation. (a) The exact probabilities
$\Ifix[,{k}](t)=I(t)^k-D(t)^k$ for $k=1,2,3,5$. (b) For two founders, the
naive power $\Ifix(t)^2=[I(t)-D(t)]^2$ lies below the exact result because it
omits mixed configurations in which one lineage has died while another remains
productive; the shaded band is precisely that missing probability. Parameters
$\lambda=1$, $\mu=0.2$, and $\delta=0.05$.}
\label{fig:N3_3}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/F3_4_conditional_mean.pdf}
\caption{Conditional-mean convergence. (a) For $\mu=0$, $\lambda=1$, and
$\delta=0.1$, $J(t)/I(t)$ and $V(t)$ both tend to
$1+\lambda/\delta=11$. (b) For $\lambda=1$, $\mu=0.2$, and $\delta=0.05$,
$J(t)/\Ifix(t)$ tends to the quasi-stationary mean $a/(a-1)$, while
$J(t)/I(t)$ tends to zero.}
\label{fig:F3_4_conditional_mean}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/N3_6_JV_parametric.pdf}
\caption{Parametric mean load--release trajectory $(J(t),V(t))$ for
$\lambda=1$, $\mu=0.2$, and $\delta=0.05$, with selected times marked along
the curve. Starting from $(1,0)$, the mean load first rises and then falls to
zero, while the expected release increases monotonically towards
$V_{\infty}=13.99$. The joint plot exposes both motions without using time as
an axis.}
\label{fig:N3_6}
\end{figure}
