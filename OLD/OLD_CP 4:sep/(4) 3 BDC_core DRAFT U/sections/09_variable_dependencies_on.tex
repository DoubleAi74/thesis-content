...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{figures/N3_2_I_economy.pdf}
\caption{The $I$-economy of the mean load $J$, mean release $V$, and second
moment $K$ for $\lambda=1$, $\mu=0.2$, and $\delta=0.05$. Time runs from left
to right as $I$ decreases from $1$ to $b$; the symmetric-log ordinate retains
the raw moment values while keeping all three curves legible. Their labelled
endpoints emphasise that time merely reparametrises a family of quantities
already fixed algebraically by $I$.}
\label{fig:N3_2}
\end{figure}
