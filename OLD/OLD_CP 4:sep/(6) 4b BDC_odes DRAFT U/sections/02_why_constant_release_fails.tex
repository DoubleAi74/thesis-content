...
least a plausible picture; for a lytic pathogen it is plainly false. It is
worth making the budding picture explicit, because it completes the
comparator that the rest of the chapter is built against.

A budding infected cell experiences two \emph{independent} event types:
release of one infectious particle, as a Poisson process of rate $p$, and
death of the cell, as a Poisson process of rate $d_{\Icell}$. The two
continue side by side until the cell dies. Its single-cell survival and
cumulative release are then
\begin{equation}
...
\label{eq:bud-cell}
\end{equation}
the mean cell lifetime is $1/d_{\Icell}$, the lifetime release is
$p/d_{\Icell}$, and the total number a cell releases is geometric on
$\{0,1,2,\ldots\}$ with
$\pr{\Kb_{\rm bud}=k}=\frac{d_{\Icell}}{d_{\Icell}+p}
\bigl(\frac{p}{d_{\Icell}+p}\bigr)^k$ --- the number of successes before
the terminating death in a sequence of events of which each is a release
with probability $p/(d_{\Icell}+p)$. All of this is contained in the two
constants $(p,d_{\Icell})$, which is exactly why the classical model is a
faithful one-cell limit for a budding pathogen.

The structural contrast with bursting is precise, and it is the engine of
everything that follows. In budding, release and death are independent
events: a cell may release many particles and then die with whatever it
contains lost, or release nothing at all. In bursting, release and death
are the \emph{same} event: whatever has accumulated is released at once,
and the cell's productive life ends in that instant. The single-cell
comparison in detail --- release laws, lifetimes, conditional means --- is
in Chapter~4a; here the point is that \eqref{eq:BMVR-classical} encodes the
budding picture exactly, and the question of this part of the chapter is
what it should be replaced with when release comes in terminal batches.
...
infected at time $t$ is $N_0\,\Ifix(t)$ (exactly in expectation, and with
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/N4b_1_constant_release_fails.pdf}
\caption{Age-dependent release compared with a constant-production
surrogate for a synchronous cohort of producer units. Both schedules have
$V_\infty=11$ and $\mathbb{E}[T]=2.398$, with
$d=1/\mathbb{E}[T]$, $p=V_\infty d$,
$(\lambda,\mu,\delta)=(1,0,0.1)$, and clearance rate $c=1$.
(a) The release kernel $g(a)=\delta K(a)$ is delayed relative to
$p\mathrm{e}^{-da}$ despite the matched lifetime yield.
(b) Linear clearance preserves the resulting early lag in the free-count
response.}
\label{fig:N4b_1}
\end{figure}
...
average the burst size over the burst-time density $\delta J$, the
density of $\tau$ derived in Chapter~4a:
\begin{equation}
...
$\delta\int\delta J(t)\,\mean{\xt^2\mid\cdot}\,\dd t$: this has the right
...
time and a burst size distributed according to the laws of Chapter~4a, and
short-duration processes occur more frequently than long ones simply
because they turn over faster. What we want
