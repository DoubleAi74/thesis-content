...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/N4b_7_kernels_three_regimes.pdf}
\caption{The exact survival and release kernels across the three flooding
regimes used in \S\ref{sec:flooding}. From left to right,
$(\lambda,\mu,\delta)=(1,0,0.1)$, $(1,0.5,1/3)$, and $(1,0.9,0.1)$ give
$L=1.10$, $1.00$, and $0.42$. The shaded areas verify
$\int_0^\infty\Ifix(a)\,\dd a=\mean{T_{\rm prod}}$ and
$\int_0^\infty g(a)\,\dd a=V_\infty$ in each column. Each release panel
has its own vertical scale so that the timing shift remains visible despite
the large yield difference: the three values of $V_\infty$ are $11.00$,
$2.00$, and $1.32$. Population conclusions therefore inherit the full
kernel shapes, not only their integrals.}
\label{fig:N4b_7}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/F4b_1_renewal_schematic.pdf}
\caption{Renewal bookkeeping for burst-aware BMVR. Infection cohorts age
from their infection times; weighting incidence by cell survival
$\Ifix(\alpha)$ and release $g(\alpha)=\delta K(\alpha)$, then summing over
ages, gives $\Icell(t)=\Icell_0\Ifix(t)+(i*\Ifix)(t)$ and
$\Vfree'(t)=\Icell_0g(t)+(i*g)(t)-c\Vfree(t)$. The feedback
$i(t)=\gamma T\Vfree(t)$ closes the population loop.}
\label{fig:F4b_1_renewal_schematic}
\end{figure}
