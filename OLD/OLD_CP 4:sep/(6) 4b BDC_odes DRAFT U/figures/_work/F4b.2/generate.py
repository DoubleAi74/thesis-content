...
def annotate_gap(ax: plt.Axes, regime: Regime) -> None:
    q_value = regime.q_representative
    burst, bud = extinction_probabilities(q_value, regime)
    burst_value, bud_value = float(burst), float(bud)
    gap = burst_value - bud_value
    ax.plot(q_value, burst_value, marker="o", ms=3.2, color="tab:blue", zorder=5)
    ax.plot(q_value, bud_value, marker="o", ms=3.2, color="tab:orange", zorder=5)
    if abs(gap) > 1e-8:
        ax.annotate(
            "",
...
def make_figure() -> plt.Figure:
    ...
    panel_letters = ("a", "b", "c")
    legend_locations = ("upper right", "upper right", "lower left")

    for panel_letter, legend_location, ax, regime in zip(
        panel_letters, legend_locations, axes, REGIMES
    ):
        _, _, flooding_parameter, v_inf = regime.derived
        q_boundary = 1.0 / v_inf
        ...
        ax.plot(q_values, burst, color="tab:blue", label="burst")
        ax.plot(
            q_values,
            bud,
            color="tab:orange",
        ...
        ax.axvline(q_boundary, color="tab:gray", linestyle=":", linewidth=1.1)
        ax.annotate(
            r"$m=1$",
            xy=(q_boundary, 1.0),
            xytext=(4, -6),
        ...
        annotate_gap(ax, regime)

        ax.set_title(
            rf"({panel_letter}) $L={flooding_parameter:.2f}$ ({regime.regime_label})"
            "\n"
            + regime.parameter_label,
            fontsize=10.5,
        )
        ax.set_xlabel(r"infection success probability $q$")
