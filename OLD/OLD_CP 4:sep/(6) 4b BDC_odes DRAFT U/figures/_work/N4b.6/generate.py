...
def make_figure(checked: dict[str, float]) -> None:
    ...
    ax.text(
        0.97,
        0.84,
        r"matched $R_0=2$, mean lifetime, and $c=1$",
        transform=ax.transAxes,
        ha="right",
        va="top",
        fontsize=8.1,
    ...
    inset = ax.inset_axes([0.59, 0.43, 0.365, 0.29])
    inset.plot(
        t,
    ...
    inset.annotate(
        "release is available at age zero",
        xy=(0.0, D_I),
        xytext=(0.75, 0.50),
    ...
    inset.set_xlabel(r"cell age $a$", fontsize=7.5, labelpad=1)
    inset.set_ylabel(r"release-age density", fontsize=7.3, labelpad=1)
    inset.set_title("Mechanism: budding begins immediately", fontsize=7.7, loc="left", pad=3)

    ax.set_xlim(0.0, 12.0)
    ax.set_ylim(0.0, max(bud_density.max(), burst_density.max()) * 1.17)
    ax.set_xlabel(r"infection generation interval $\tau$")
    ax.set_ylabel(r"normalized generation-time density $w(\tau)$")
    ax.set_title(
        "Budding shifts transmission towards earlier generations",
        loc="left",
        fontweight="bold",
        pad=9,
    )
