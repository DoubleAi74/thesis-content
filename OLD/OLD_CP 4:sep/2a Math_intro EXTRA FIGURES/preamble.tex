...
% Full-measure yellow boxes (never wider than \linewidth) + red number badge.
% Toggle off with:
...
% Full-measure yellow box on its own line(s).  \leavevmode first so that a
% run-in \paragraph/\subparagraph title is materialised before we \par, and
% the box is never placed on the same line as that title (which was the
% source of ~150pt overfulls).  Width subtracts both \fboxsep pads plus a
% 1pt safety margin.
\newcommand{\hln}[2]{%
  \leavevmode
  \ifhmode\unskip\fi
...
% Block form for display math (same width discipline).
\newcommand{\hlnbox}[2]{%
  \par\nopagebreak
  \noindent
