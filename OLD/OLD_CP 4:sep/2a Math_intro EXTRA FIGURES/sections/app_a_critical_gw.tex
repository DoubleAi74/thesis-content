...
the conditioned population does.
...
= \sum_{n=0}^{\infty}S_n .
\end{equation}
Everything therefore turns on how fast $S_n$ decays at criticality.
...
systematic pattern in longevity and diversity data \cite{Budd2018}. The same
conditioning produced the quasi-stationary mean of \cref{m:sec:condmeans}: there it
holds a decaying mean up, here it inflates an early growth rate, and in both the
selection is performed on trajectories after they have been generated.
...
\cref{m:eq:condvar} without deriving it. The derivation is short, and worth having:
the mean of a distribution says nothing about whether the distribution itself is
settling down, and a second moment that also stabilises is the least one should ask
of a population claimed to be quasi-stationary.
...
\end{equation}
the geometric factors cancelling exactly. Both conditional moments therefore
converge, and subtracting the square of the first from the second gives
\cref{m:eq:condvar}.

The limit \cref{m:eq:condsecondmoment} is again a function of $p$ and the single
constant $A(p)$ alone, so the second moment adds no new unknown.
