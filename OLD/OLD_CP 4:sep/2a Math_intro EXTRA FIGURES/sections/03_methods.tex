...
\cref{m:eq:SnIter}, and the method that actually resolves that question --- the
Koenigs linearisation, which conjugates the map to a multiplication and turns the
orbit into a geometric sequence --- is the subject of \ChA.
...
ground for the multi-type models of later chapters; it is not developed here.
...
S_n\sim A(p)\,(2p)^n .
\end{equation}
We call $A(p)$ the \emph{amplitude} of the decay. The amplitude accumulates every
early nonlinear correction made before
...
is why the elementary methods of this subsection could not have succeeded.
...
\end{equation}
again a function of $p$ and $A(p)$ alone; the second moment behind it is computed in
\cref{m:app:condvariance}. \Cref{m:fig:condmean} shows the
...
reached only $24.1$ of its limiting $27.5$.}
...
\ChAcap{} proves the strict bound
...
this function is hypertranscendental. For the purposes of the present chapter, two
facts suffice and both are established there: the limit \cref{m:eq:Apdef} exists
with
