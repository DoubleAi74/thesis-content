...
def main() -> None:
    ...
    # --- figure ---
    fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(7.13, 2.82))

    colors = style_rc.ramp(len(ks_flux))
    ...
    ax1.plot(ks_mean, cond, "o-", color=style_rc.BLUE, lw=2.0, markersize=6, label=r"$V_\infty^{(k)}/(1-b^k)$")
    ...
    fig.tight_layout()
    out_pdf = FIGDIR / (FIGSTEM + ".pdf")
    preview = FIGDIR / (FIGSTEM + ".png")
