...
realisations are absorbed into the separate state $R$ and no longer
...
state, this summation term is not present, and the first equation holds for
all $i\ge0$ with $p_0(t)\equiv0$.\par
\vspace{10pt}
...
$$\pddt{G(z,t)} = \delta J(t) + \beta z^2 \sum^\infty_{i=1} ip_iz^{i-1} + \mu\sum^\infty_{i=1}ip_iz^{i-1} - (\beta +\mu+\delta)z\sum^\infty_{i=1}ip_iz^{i-1}$$
