...
% (not \alpha/\beta); eclipse division \rho_{\rm div} (not \rho).
...
\foreach \x/\lab/\side in {%
  ...
  8.8/{boolean thinning:\\ partial clear, $q\in(0,1)$}/below,
...
catastrophe), a tunable fraction $q$ per event (boolean thinning),
...
Absorbing BDC (this chapter) & $\Ihat(a)$ & $\delta K(a)$ \\
MOI-$k$ BDC (\S\ref{sec:moi}) & $I(a)^k-D(a)^k$ & $\delta K_k(a)$ \\
...
Two-type / GATE (Ch.~5) & two-type $\Ihat$ & two-type second-moment flux \\
\bottomrule
\end{tabular}
\end{center}
...
\S\ref{sec:moi}. The mature quasi-steady balance,
...
the load undergoes birth $\beta n$, death $\mu n$, optional immigration
...
\midrule
After dump & process ends in $R$ & $X\to0$, continues \\
Dumps per cell & $\le1$ & many, until cell death \\
Survival kernel & $\Ihat$ & $e^{-d_{\Icell}a}$ \\
...
\end{tabular}
\end{center}
They nest: Model~2, with intensity $\lambda(t)=f(X_t)+\sum_{t_i<t}\phi(t-t_i)$,
is the general event-history form --- $\phi\equiv0$ and $f(x)=\varepsilon x$
recovers continuous export (Case~3); $\phi\equiv0$, $f(x)=\delta x^2$ with
full clear recovers Model~7 and Cases~4--5; an exponential
$\phi(t)=\eta e^{-\kappa t}$ is equivalent to Model~10; Model~6 is a coarse
...
decays to baseline $r_0$ at rate $\kappa$:
$\dd r=-\kappa(r-r_0)\dd t+\eta\,\dd N_t$, with flux $r(t)$ or $r(t)X_t$.
...
$\lambda(t)=f(X_t)+\sum_{t_i<t}\phi(t-t_i)$; on an event $X$ drops by the
...
form exists only after reduction to exponential $\phi$, i.e.\ to
...
$\to$ classic $p$ & QSS $\delta m^2$ & duty cycle & QSS $\mean{rX}$ & as 10 if exp.\ $\phi$ \\
\bottomrule
\end{tabular}
\end{center}
...
released independently with probability $q$,
$B\mid X=n\sim\mathrm{Binomial}(n,q)$; $q\to0$ tiny fractional clear,
$q=1$ full clear. Generating functions stay friendly.
\item[B.] Fixed fraction $B=\mathrm{round}(fn)$: same mean as A with
$q=f$, less stochasticity.
...
rate $\lambda(X)$ with $B\sim\mathrm{Binomial}(X,q)$ released and $X\to
...
Choice & Recovers \\
\midrule
$q=1$, $\lambda=\delta X^2$ & full clear, cooperative (Case~4 style) \\
$q=1$, $\lambda=\delta X$ & full clear, linear hazard \\
$0<q<1$, $\lambda=\delta X$ & the main partial-clear intermediate \\
...
transparent parameter $q$ between mild partial clear and full dumping,
...
burst-size noise is wanted. The flooding boundary as a function of $q$ ---
