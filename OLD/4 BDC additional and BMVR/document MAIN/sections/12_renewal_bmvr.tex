...
% Numerics: new_notes3/verify_result_20_1.py (53/53 PASS);
...
\begin{equation}
S(a):=\Ihat(a)=\frac{(a-b)^2\,\ee^{\theta a}}
...
\caption{The cell-survival kernel $\Ihat(a)$ and the release kernel
...
such a cell is still productive with probability $\Ihat(\alpha)$ and
...
\label{res:new-vde}
\begin{equation}
\Icell(t)=\Icell_0\,\Ihat(t)+\int_0^t i(t-\alpha)\,\Ihat(\alpha)\,\dd\alpha
=\Icell_0\,\Ihat+(i*\Ihat)(t),
\label{eq:Icell}
\end{equation}
\begin{equation}
\ddt{\Vfree}(t)
...
convolution with $g$. The original attempt of Chapter~4 only ever proposed
to change the second.

Differentiating \eqref{eq:Icell} makes the structure visible:
\begin{equation}
\ddt{\Icell}
= i(t)+\Icell_0\,\Ihat'(t)+\int_0^t i(t-\alpha)\,\Ihat'(\alpha)\,\dd\alpha,
\end{equation}
...
the framework. It is that the kernels $\Ihat$ and $g$ are \emph{derived in
...
\eqref{eq:Vfree} collapse to $\Icell=\Icell_0\Ihat$ and the forced linear
...
($\beta=1$, $\mu=0$, $\delta=0.1$). Solid navy: renewal; dashed orange:
