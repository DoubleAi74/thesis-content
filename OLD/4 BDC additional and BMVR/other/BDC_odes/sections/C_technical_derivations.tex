% Appendix C (Chapter 4b) --- hypergeometric Laplace transforms of the
% kernels. The V_inf^(k) derivation and the PGF coefficient extraction live
% in Chapter 4a's Appendix C.
...
Write $s:=r/\theta$ and recall $\theta=\lambda(a-b)$. Both transforms reduce,
...
\eqref{eq:dLK} gives $\delta\Lap{K}(0)=(\delta/\lambda)(1/A+1/A^2)
...
PGF coefficient extraction used by the chained model --- are in
Chapter~4a's Appendix~C.
