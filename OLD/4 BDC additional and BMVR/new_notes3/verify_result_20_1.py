...
def main():
    ...
    ax[0].plot(t, Vr, "b-", lw=2, label="renewal")
    ax[0].plot(t, Vc, "r--", lw=2, label="classical ODE")
    ax[0].set_xlabel("t")
    ax[0].set_ylabel(r"$\mathcal{V}(t)$")
    ax[0].set_title("Exponential kernels: free virus")
    ax[0].legend()
    ax[1].plot(t, Ir, "b-", lw=2, label="renewal")
    ax[1].plot(t, Ic, "r--", lw=2, label="classical ODE")
    ax[1].set_xlabel("t")
    ax[1].set_ylabel(r"$\mathcal{I}(t)$")
    ax[1].set_title("Exponential kernels: infected cells")
    ax[1].legend()
    fig.tight_layout()
    ...
    ax.semilogy(t, V, "b-", lw=2, label=r"$\mathcal{V}(t)$ renewal")
    # reference slope
    t_ref = t[t > 0.5 * t[-1]]
    V_ref = V[t > 0.5 * t[-1]][0] * np.exp(r_star * (t_ref - t_ref[0]))
    ax.semilogy(t_ref, V_ref, "k--", lw=1.8, label=rf"$e^{{r t}}$ with $r=r_*={r_star:.4f}$")
    ax.set_xlabel("t")
    ax.set_ylabel(r"$\mathcal{V}(t)$")
    ax.set_title("TEST E: asymptotic growth vs characteristic root")
    ax.legend()
    fig.tight_layout()
    ...
    for R0 in R0_list:
        ...
        ax.semilogy(t, V + 1e-16, label=f"R0={R0:.2f}")
        rep.add(
            f"F.threshold[R0={R0}]",
            ok_case,
    ...
    ax.set_xlabel("t")
    ax.set_ylabel(r"$\mathcal{V}(t)$")
    ax.set_title(r"Test F: $R_0$ threshold scan ($\mu=0,\delta=0.1$)")
    ...
    for scn in stoch_cases:
        ...
        fig, ax = plt.subplots(1, 2, figsize=(9, 3.5))
        ax[0].plot(t, I_ren, "b-", lw=2, label="renewal (mean-field)")
        ax[0].plot(t, I_st, "r--", lw=2, label="Gillespie ensemble mean")
        ax[0].set_xlabel("t")
        ax[0].set_ylabel(r"$\mathcal{I}(t)$")
        ax[0].set_title(f"Infected cells ({scn['name']})")
        ax[0].legend(fontsize=8)
        ax[1].plot(t, V_ren, "b-", lw=2, label="renewal (mean-field)")
        ax[1].plot(t, V_st, "r--", lw=2, label="Gillespie ensemble mean")
        ax[1].set_xlabel("t")
        ax[1].set_ylabel(r"$\mathcal{V}(t)$")
        ax[1].set_title(f"Free virus ({scn['name']})")
        ax[1].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(os.path.join(FIGDIR, f"H_gillespie_{scn['name']}.pdf"))
        plt.close(fig)
