...
alternative is to integrate the quotient \cref{eq:xabd} directly, splitting the
...
\end{equation}
The remaining integrand matches that of \cref{eq:idenHyp} with $b = h/k$,
...
\end{equation*}
Substituting into \cref{eq:backat} gives
\begin{equation*}
...
\begin{remark}
Applied to the two pieces of \cref{eq:xabd} --- the first with $h=\alpha$, the
second with $h = \alpha+b_1$, and both with $k=b_1$, $P = \sigma-x_-$ and
$Q = -(\sigma-x_+)$ --- \cref{prop:hypIden} reproduces \cref{eq:Psihyp} once the
...
\cref{eq:absdeathstates}, so that
\begin{equation*}
p_{i,j}(t) = \frac{n!}{i!\,j!\,(n-i-j)!}\;f(t)^{i}g(t)^{j}c(t)^{\,n-i-j} .
\end{equation*}
...
factorises in this way is \cref{eq:indRelPGF} read backwards.

The absorption--birth--death generating function of \cref{prop:Gabd} is not of the
form \cref{eq:multilinear}, and there \cref{eq:stateProb} must be applied to
