...
The separated special-function form is compact, but a direct conditioning formula
remains transparent at \(\lambda=\mu\), at resonance and across hypergeometric
branch cuts.  Let \(F(x,t)\) be the PGF of an interior linear birth--death process
started from one particle.  Its backward equation is
\begin{equation}
 \pddt{F}=\lambda F^2-(\lambda+\mu)F+\mu,
 \qquad F(x,0)=x.
 \label{m:eq:interior-bd-backward}
\end{equation}
For \(\lambda\ne\mu\),
\begin{equation}
 \frac{F(x,t)-1}{F(x,t)-\mu/\lambda}
 =\frac{x-1}{x-\mu/\lambda}\ee^{(\lambda-\mu)t},
 \label{m:eq:interior-bd-ratio}
\end{equation}
whereas at \(\lambda=\mu\),
\begin{equation}
 F(x,t)=1-\frac{1-x}{1+\lambda t(1-x)}.
 \label{m:eq:interior-bd-critical}
\end{equation}
For one initial exterior particle, either transfer has not occurred by time \(t\),
or it occurs at time \(u\in(0,t)\) and leaves an interior process of duration
\(t-u\).  Conditioning on that transfer time gives
\begin{equation}
 g(x,y,t)=\ee^{-\alpha t}y
 +\alpha\ee^{-\alpha t}\int_0^t\ee^{\alpha v}F(x,v)\,\dd v,
 \qquad G(x,y,t)=g(x,y,t)^N.
 \label{m:eq:abd-convolution}
\end{equation}
This representation is real and finite for \(x,y\in[0,1]\) and all positive
rates.  It also makes the basic checks immediate: \(G(x,y,0)=y^N\),
\(G(1,1,t)=1\), and
\(G(1,y,t)=\{\ee^{-\alpha t}y+1-\ee^{-\alpha t}\}^N\).
...
\subsubsection*{Recovering state probabilities}
For each model above, coefficients recover the state law:
\begin{equation}
 p_{i,j}(t)=[x^iy^j]G(x,y,t)
 =\frac{1}{i!j!}\left.
 \frac{\partial^{i+j}G}{\partial x^i\partial y^j}\right|_{x=y=0}.
 \label{m:eq:bivariate-coefficient}
\end{equation}
In the absorption--birth--death model, write
\[
 h(x,t)=\alpha\ee^{-\alpha t}\int_0^t\ee^{\alpha v}F(x,v)\,\dd v,
 \qquad g(x,y,t)=\ee^{-\alpha t}y+h(x,t).
\]
The finite exterior expansion separates the two coefficient extractions:
\begin{equation}
 p_{i,j}(t)=\binom Nj\ee^{-\alpha tj}[x^i]h(x,t)^{N-j},
 \qquad 0\le j\le N.
 \label{m:eq:abd-coefficient-formula}
\end{equation}
The remaining univariate coefficient may be obtained by differentiation,
convolution of truncated series, or Cauchy's integral formula.  The appendix gives
the corresponding extraction in the simpler multilinear cases.
