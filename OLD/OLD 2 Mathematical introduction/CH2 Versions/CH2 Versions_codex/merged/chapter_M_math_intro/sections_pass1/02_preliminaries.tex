...
\subsection{First-step analysis}\label{m:sec:first-step}

When the target is a hitting probability or a mean hitting time rather than a full
time-dependent law, first-step analysis conditions on the first exit from the
current state and solves the resulting linear system.  This is the same operation
seen from three directions: conditioning on the exponential clock that wins a
race, writing a backward equation for a continuous-time chain, or obtaining the
Galton--Watson extinction fixed-point equation after the first reproduction event.
One step of randomness is exposed, after which the unknown quantity reappears in a
