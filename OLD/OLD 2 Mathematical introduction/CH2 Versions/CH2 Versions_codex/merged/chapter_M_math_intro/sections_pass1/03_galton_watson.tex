...
\subsection{Critical total progeny}\label{m:sec:total-progeny}

The total progeny \(\mathcal T=\sum_{n\geq0}Z_n\) has a different critical
exponent from the extinction time.  A finite binary genealogy with \(k\) division
vertices contains \(k+1\) terminal vertices and hence \(2k+1\) individuals in all.
The number of ordered full binary trees of this size is the Catalan number
\(C_k=(k+1)^{-1}\binom{2k}{k}\).  Therefore
\begin{equation}
  \pr{\mathcal T=2k+1}=C_kp^k(1-p)^{k+1}.
  \label{m:eq:total-progeny-exact}
\end{equation}
At criticality, Stirling's formula yields
\begin{equation}
  \pr{\mathcal T=2k+1}\sim\frac{1}{2\sqrt{\pi}}k^{-3/2},
  \label{m:eq:total-progeny-asymptotic}
\end{equation}
with zero probability at even values.  This is the \(3/2\) law displayed in the
last panel of \cref{m:fig:power_law}.

The survival recursion and the critical exponents above are elementary
consequences of generating-function iteration.  What is not elementary is the
parameter dependence of the subcritical normalising constant \(A(p)\); that
separate analytic problem is the subject of \ChA.
