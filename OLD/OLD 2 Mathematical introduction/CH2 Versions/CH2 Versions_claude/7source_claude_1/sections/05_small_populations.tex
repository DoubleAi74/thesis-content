...
Return to the binary process of \cref{sec:deathordivision} and suppose it is near
...
\Cref{fig:earlySn} makes the point: whatever advantage supercriticality confers, it
...
progenitor, computed from \cref{eq:ext}. Even in the supercritical case
...
\caption{As \cref{fig:kvals}, but with $p$ displaced slightly from criticality:
...
nearly coincident in \cref{fig:kvals}.}
\label{fig:kvals2}
\end{figure}

\Cref{fig:kvals,fig:kvals2} show $S_n^{(k)}$ through $500$ generations for
$k=1,10,100,1000$. The effect visible in \cref{fig:kvals} --- that a larger first
...
dramatically into supercriticality, and the two panels of \cref{fig:kvals2} differ
only in the third decimal place of $p$.

\subsection{Conditioning and apparent early growth}
\label{sec:earlyrep}

The conditioning of \cref{sec:qsd} has a mirror image which is worth recording,
...
have escaped the early extinction risk documented in \cref{fig:earlySn}, and so far
...
That is exactly the conditioning of \cref{sec:qsd} seen from the other side. There
...
rates are unchanged. \Cref{fig:earlySn} is the reason to take it seriously, since
...
\cref{eq:bdcguess} is better read as a lower bound than as an equality. How tight a
bound remains a question for numerical work.

\subsection{Continuous-time birth--death}
\label{sec:cts}

The discrete process resists a closed form for $A(p)$, as \cref{sec:Ap} shows at
length. Its continuous-time counterpart does not, and the comparison between the
two turns out to be informative, so the continuous-time constant is recorded here.

Take the linear birth--death process of \cref{sec:linbd}, with per-capita birth
rate $\lambda$ and death rate $\mu$, started from $N$ individuals. Its extinction
probability $p_0(t)$ is \cref{eq:p0t}, and its survival probability is
$S^{(N)}(t) = 1-p_0(t)$.
...
subcritical case. Exactly as in \cref{sec:condmean},
\begin{equation}
\label{eq:condit}
...
matched, and \cref{prop:race} is what matches them: with birth and death clocks
...
the discrete-time $A(p)$ of \cref{eq:lateS}.
...
\Cref{fig:dtct} plots $\Ac(p)$ against numerically obtained values of $A(p)$, and
...
at the other end of the range they meet. The answer is given in \cref{sec:compare},
once the necessary machinery is in place; it turns out that $\Ac(p)/2$ is an exact
lower bound for $A(p)$, attained only in the limit $p\to0$.

\subsection{Continuous-time birth--death--catastrophe}
\label{sec:ctbdc}

The discrete process of \cref{sec:bdc} has a continuous-time counterpart, and since
...
Two features distinguish \cref{def:bdc} from the linear birth--death process of
\cref{sec:linbd}, and both matter later. The process now has \emph{two} absorbing
...
The generating-function machinery of \cref{sec:pgf} applies unchanged. Restricting
...
\cref{eq:bdcrates} yields
\begin{equation}
\label{eq:bdcpde}
\pddt{G} = \bigl(\lambda z^2-(\lambda+\mu+\rho)z+\mu\bigr)\pdd{G}{z},
\qquad G(z,0) = z^{N},
\end{equation}
which differs from \cref{eq:bdpde} only in the coefficient of $z$. The quadratic no
...
\cref{eq:bdcpde} and its relatives is the subject of \cref{sec:moc}.
