...
nineteenth century, it remains the fundamental model of stochastic branching
\cite{Athreya1972,Harris1963}.

Suppose each individual produces, independently of every other, a number of offspring
distributed as the random variable $\xx$, with
\begin{equation*}
\pr{\xx = k} = p_k,\qquad k = 0,1,2,\dots,
\end{equation*}
and write $\mu = \mean{\xx}$ for the mean number of offspring per individual. Let
$\zz_n$ be the number of individuals in the $n$th generation, with a single progenitor
$\zz_0=1$. Conditioning on the previous generation and using the tower property,
\begin{equation*}
\mean{\zz_{n+1}} = \mean{\mean{\zz_{n+1}\mid \zz_n}} = \mean{\mu\,\zz_n} = \mu\,\mean{\zz_n},
\end{equation*}
so that $\mean{\zz_n} = \mu^n$. If $\mu<1$ the expected population decays geometrically
and $\lim_{n\to\infty}\pr{\zz_n=0}=1$: extinction is certain.
...
first-step argument yields the iteration on which almost everything in this chapter
depends,
\begin{equation}
\label{iterPGF}
u_{n+1} = \phi(u_n).
...
Throughout we specialise to the binary --- or simple --- case, in which an individual
either divides into two or dies:
\begin{align*}
p_2 &= p &&\text{(division)},\\
p_0 &= 1-p &&\text{(death)},\\
p_i &= 0 &&\forall\, i\neq 0,2.
\end{align*}
The mean number of offspring is then $\mu = 2p$, and the expected population descended
from a single ancestor is
\begin{equation}
\label{eq:meanZn}
\mean{\zz_n} = (2p)^n.
\end{equation}
The process is subcritical when $2p<1$, critical when $p=\tfrac12$, and supercritical
when $2p>1$. \Cref{fig:gwvis} shows realisations either side of the critical point.
...
happens with probability $1-p$, extinction has already occurred. If instead it divides,
with probability $p$, then extinction by generation $n+1$ requires each of the two fresh
lineages to die out within its own $n$ generations, which happens with probability
$u_n^2$.
...
\end{equation}
Since a population cannot return from extinction, $u_n$ is non-decreasing and bounded
...
\end{equation*}
For $p<\tfrac12$ the second root is negative and so carries no probabilistic meaning;
the only admissible limit is $S_\infty=0$ and extinction is certain. At $p=\tfrac12$ the
two roots collide at the origin, and extinction remains certain. For $p>\tfrac12$ a
positive root appears, $S_\infty = (2p-1)/p$, and it is this root that is attained: the
process survives forever with positive probability. \Cref{fig:phase_transition} shows
the transition.
...
At $p=\tfrac12$ the expectation is constant, $\mean{\zz_n} = 1$ for all $n$, and yet
extinction is certain. The reconciliation is that the expected lifetime is infinite:
rare realisations survive for enormously long times and grow enormously large, and they
do so at exactly the rate needed to hold the mean at one. The same calculation supplies
the asymptotics of $S_n$ that \cref{sec:Ap} will need.
...
\end{equation}
The question becomes: how fast does $S_n$ go to zero at criticality?
...
which $S_n$ varies, and the difference on the left is well approximated by a derivative:
\begin{equation*}
...
\end{equation*}
which diverges. Since every $S_n$ is positive, $\mean{T}=\infty$. The expected lifetime
of the critical Galton--Watson process is infinite even though extinction occurs with
probability one.
...
The heavy tail responsible for this is visible directly in simulation. For a power-law
distributed variable there is a modest but non-negligible chance of an extraordinarily
large realisation, and a sample mean computed from such a variable does not settle: the
larger the sample, the larger the mean, because the occasional enormous value keeps
pushing the normalised sum upward. \Cref{fig:power_law} collects the three classical
critical exponents.
