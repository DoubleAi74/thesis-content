...
\begin{abstract}
\noindent
For the binary Galton--Watson process with division probability $p\in[0,\tfrac12)$, the survival probability $S_n$ satisfies $S_n\sim A(p)\,(2p)^n$, so the limiting mean population size conditional on non-extinction is $1/A(p)$. We develop the basic branching background, the definition and properties of $A(p)$, an infinite-product representation, near-critical asymptotics, and numerical/search attempts at a closed form. We then identify $A(p)$ with (twice) the Koenigs function of the logistic map $f_r(w)=rw(1-w)$ at $w=\tfrac12$ and multiplier $r=2p$. Drawing on classical linearisation theory and the hypertranscendence results of Becker and Bergweiler, we explain why no elementary closed form for the conjugacy---and hence none expected for $A(p)$---exists for $r\in(0,1)$, while recording the exceptional elementary cases $r=2$ and $r=4$ (outside the branching range) in an appendix. The claim is stated carefully: fixed-multiplier non-elementarity of the conjugacy underpins the obstruction; a full classification of the scalar map $p\mapsto A(p)$ is not claimed.
\end{abstract}
...
The analysis of this section follows the note on hypertranscendence of the logistic Koenigs function \cite{Koenigs1884,Becker1995}, integrated here as the natural continuation of the $A(p)$ story.

\subsection{Statement}

For $r\in(0,1)$ the origin is an attracting fixed point of $f_r$ with multiplier $\lambda=r$. Classical linearisation supplies $\psi_r$ as in Definition~\ref{def:koenigs}. Results of Becker and Bergweiler on hypertranscendence of conjugacies for rational maps \cite{Becker1995} imply that such linearisations are differentially algebraic only for highly exceptional normal forms (monomial or Chebyshev type). Under the standard affine conjugacy of $f_r$ to $z^2+c$ with
\begin{equation}
\label{eq:c-of-r}
c(r)=\frac{2r-r^2}{4},
\end{equation}
the interval $r\in(0,1)$ maps to $c\in\bigl(0,\tfrac14\bigr)$, which avoids the exceptional quadratic parameters $c\in\{0,-2\}$ associated with $r=2$ and $r=4$. Therefore, for each fixed $r\in(0,1)$, the conjugacy $\psi_r$ is hypertranscendental as a function of $z$: it satisfies no algebraic differential equation and is not elementary in the Liouville sense.
...
\subsection{Consequences for $A(p)$}

Combined with \eqref{eq:A-koenigs}, extracting $A(p)$ cannot proceed by evaluating a single elementary expression for $\psi_{2p}$ as a function of $z$. Two logical caveats remain if one wants a stronger theorem:
\begin{enumerate}[leftmargin=*]
\item Non-elementarity of $z\mapsto\psi_r(z)$ does not by itself forbid a closed form for one value $\psi_r(\tfrac12)$ (compare $\Gamma$ and $\Gamma(\tfrac12)=\sqrt{\pi}$).
\item Elevating the statement to a full classification of the scalar map $p\mapsto A(p)$ is a further step beyond fixed-$r$ hypertranscendence.
\end{enumerate}
The working claim of this note is therefore:
...
Full derivations of the elementary solutions at $r=2$ and $r=4$, and a more detailed account of the conjugacy and the Becker--Bergweiler dichotomy, are collected in Appendix~\ref{app:integrable}.
...
The constant $A(p)$ organises the quasi-stationary mean of subcritical binary Galton--Watson branching. It is well defined, computable, and linked to the basin Koenigs value $A(p)=2\lim r^{-n}f_r^{\circ n}(\tfrac12)$ at multiplier $r=2p$. The hypertranscendence theory for that conjugacy on $r\in(0,1)$ explains the failure of elementary closed-form searches, while leaving $A(p)$ fully usable for analysis and numerics. Exceptional elementary linearisations at $r=2$ and $r=4$ exist but lie outside the branching window; their derivations appear in the appendix.
...
For $r=4$, $f_4(x)=4x(1-x)$. With $x=\sin^2\theta$ one finds $f_4(x)=\sin^2(2\theta)$, so the angle doubles. The candidate
\begin{equation}
\label{eq:phi4}
...
Under this conjugacy, $r=2\Leftrightarrow c=0$ (monomial $z^2$) and $r=4\Leftrightarrow c=-2$ (Chebyshev type). Neither exceptional value lies in the image of $r\in(0,1)$, which is $c\in\bigl(0,\tfrac14\bigr)$.

\subsection{Becker--Bergweiler dichotomy (summary)}

\begin{theorem}[Becker and Bergweiler \cite{Becker1995}]
\label{thm:BB}
For a rational map of degree $d\ge 2$, the linearisation at a fixed point with multiplier $\lambda$ in the classical non-resonant range is differentially algebraic only in exceptional normal forms (monomial / Chebyshev type up to analytic conjugacy).
\end{theorem}

Applied to the quadratic family via Lemma~\ref{lem:affine}, the only elementary logistic parameters of this type are $r=2$ and $r=4$, both outside $r\in(0,1)$. Hence for each fixed $r\in(0,1)$ the conjugacy is hypertranscendental as a function of $z$, which underpins the main-text claim for $A(p)=2\psi_{2p}(\tfrac12)$.
...
vol.~301, pp.~463--468, 1995.
% Note: page/range conventions vary across printings; see also related AMS Proc.\ articles by the same authors.
