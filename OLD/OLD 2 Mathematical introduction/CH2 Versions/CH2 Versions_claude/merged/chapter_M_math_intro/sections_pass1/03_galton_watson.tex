...
Generating functions convert \cref{m:eq:gwrecursion} into an iteration. Write
$\phi(z) = \ex{z^{L}}$ for the \pgf{} of the offspring law. Because $Z_{n+1}$ is a
random sum of independent copies of $L$, the composition rule of \cref{m:sec:pgf}
...
\end{equation}
This is the discrete-time first-step principle of \cref{m:sec:firststep}: condition
on the founder's offspring, and require each resulting lineage to die independently.

\subsection{Death or division}
\label{m:sec:deathordivision}
...
and \emph{supercritical} when $2p>1$. \Cref{m:fig:gwvis} shows realisations either
side of the critical point.

\begin{figure}[ht]
\centering
\includegraphics[width=0.78\textwidth]{simpleGWvis.png}\\[6pt]
...
\end{equation}
so that \cref{m:eq:iterPGF} reads
\begin{equation}
\label{m:eq:unIter}
u_{n+1} = p\,u_n^2 + 1-p .
\end{equation}
The recursion has a direct combinatorial reading, and it is worth taking a moment
over it because the same argument recurs in \cref{m:sec:cohort,m:sec:bdc}. If the
...
$u_n = 1-S_n$ into \cref{m:eq:unIter} and simplifying gives
\begin{equation}
\label{m:eq:SnIter}
...
Since $S_n$ converges, its limit must be a fixed point of \cref{m:eq:SnIter}. Setting
$S_{n+1}=S_n=S_\infty$,
\begin{equation*}
...
= \sum_{n=0}^{\infty}S_n .
\end{equation}
The question is therefore how fast $S_n$ decays at criticality. It decays like
$2/n$, and this can be established directly from the exact recursion, with no
...
\end{equation}
By \cref{m:sec:certainty}, $S_n\downarrow0$, so the reciprocal sequence tends to
...
\begin{remark}[The continuum route]
\label{m:rem:continuumcritical}
The same asymptotic is reached more quickly, and less rigorously, by treating $n$ as
continuous. Once $n$ is large the unit generational increment is small compared with
the scale on which $S_n$ varies, and the difference
$S_{n+1}-S_n = -\tfrac12S_n^2$ is well approximated by a derivative:
\begin{equation*}
\dda{S}{n} = -\tfrac12 S^2,
...
correct. The same substitution is made again in \cref{m:sec:riccati}, where it
cannot be repaired so easily.
\end{remark}

Substituting \cref{m:eq:twoovern} into \cref{m:eq:ETsumSn}, the tail
...
\begin{equation}
\label{m:eq:totalprogeny}
\mathcal T = \sum_{n\ge0}Z_n .
\end{equation}
A finite binary genealogy with $k$ division vertices carries $k+1$ terminal
vertices, hence $2k+1$ individuals in all, and the number of ordered full binary
trees with $k$ internal vertices is the Catalan number
$C_k = (k+1)^{-1}\binom{2k}{k}$. Each such tree is realised with probability
$p^k(1-p)^{k+1}$, so
\begin{equation}
...
enormous value keeps pushing the normalised sum upward. \Cref{m:fig:power_law}
collects the three critical exponents --- for the survival probability, for the
extinction time and for the total progeny --- of
\cref{m:eq:twoovern,m:eq:criticalTtail,m:eq:totalprogenyasym}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.98\textwidth]{power_law_fixed.pdf}
...
\subsection{What is elementary here, and what is not}
\label{m:sec:elementaryornot}

It is worth pausing on the contrast the last two subsections have set up, because
it determines the division of labour between this chapter and the next. At
criticality every quantity of interest has an elementary description: the survival
probability decays like $2/n$ by \cref{m:prop:criticalSn}, the extinction-time tail
like $2/n^2$, the total progeny law is a Catalan expression in closed form. Move
off criticality, into the subcritical interior $p\in(0,\tfrac12)$, and the picture
inverts. The decay of $S_n$ is geometric, $S_n\sim A(p)(2p)^n$, and the rate $2p$ is
immediate --- but the amplitude $A(p)$, which is the only thing the limiting
conditional law depends on, has no known elementary expression at all, and there is
a structural reason to expect none. That amplitude is defined in \cref{m:sec:qsd},
used throughout the rest of this chapter as a named object, and analysed in \ChA.
