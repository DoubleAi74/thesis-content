...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{abs1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{abs2.pdf}
...
probability in \cref{m:sec:riccati}. Its right-hand side factorises, and the roots
...
\label{m:sec:abdregular}

The special-function form \cref{m:eq:Gabd} is compact, but its derivation separated
an antiderivative into pieces that are individually singular at certain parameter
values. The underlying probability calculation supplies a second representation
that stays finite everywhere, and it is worth recording because it settles the
questions of domain and resonance below without any analytic continuation.
...
on the absorption time and reversing the direction of the inner integral,
\begin{equation}
\label{m:eq:abdconvolution}
g(x,y,t) = \ee^{-\alpha t}y
+ \alpha\ee^{-\alpha t}\int_0^t\ee^{\alpha v}F(x,v)\,\dd v ,
\end{equation}
and since the $N$ exterior particles are independent up to the point of absorption
and independent thereafter,
\begin{equation}
\label{m:eq:abdconvpower}
G(x,y,t) = g(x,y,t)^N .
\end{equation}
\Cref{m:eq:abdconvolution} is real and finite for $x,y\in[0,1]$ and all positive
rates, including $\lambda=\mu$. For $\lambda\neq\mu$, substituting
\cref{m:eq:interiorratio} into the integral and integrating term by term as above
returns \cref{m:eq:Gabd}; so \cref{m:eq:abdconvolution} also specifies unambiguously
which continuation of the special-function expression is intended.

Several of the checks above now need almost no algebra. At $t=0$ the integral
vanishes and $g=y$. At $x=y=1$ we have $F(1,v)=1$, so $g=1$ and probability is
conserved. Setting $x=1$ gives $g = \ee^{-\alpha t}y+1-\ee^{-\alpha t}$ and the
binomial exterior marginal directly.

\subsubsection{Hypergeometric form, domain and resonance}
\label{m:sec:abdhyp}
...
\cref{m:eq:abdconvolution} has no singular denominator at all and supplies the limit
directly. Resonance is therefore a defect of the separated antiderivative, not of
the generating function; the resonant parameter values form a set of measure zero,
but the degeneracy should be kept in mind when $\alpha$ happens to sit near an
integer multiple of $\mu-\lambda$.
\end{remark}

\subsection{Recovering state probabilities}
\label{m:sec:coefrecovery}
...
\begin{equation}
\label{m:eq:abdcoef}
p_{i,j}(t) = \binom{N}{j}\ee^{-\alpha t j}\,[x^i]\,h(x,t)^{\,N-j},
\qquad 0\le j\le N .
\end{equation}
