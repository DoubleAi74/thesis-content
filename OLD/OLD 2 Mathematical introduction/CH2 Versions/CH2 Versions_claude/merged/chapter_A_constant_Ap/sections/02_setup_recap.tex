...
This section fixes what is carried over from \ChM. It is deliberately short: the
probabilistic development belongs there, and only the objects this chapter operates
on are restated.
...
\cref{a:sec:product}. The constant is not accessible to any argument confined to
late generations, because it is precisely the accumulated effect of the early
nonlinear corrections, before the recursion attenuates to $S_{n+1}\approx2pS_n$.

Two consequences of \cref{a:eq:lateS} are the reason the constant matters. Splitting
...
\end{equation}
Unlike $A(p)$, the continuous-time constant $\Ac(p)$ is elementary. Much of what
follows is an account of the exact relationship between the two, and of why the
discrete one is harder.
