...
The argument descends from the concrete to the structural, and each stage
constrains the one after it. After a short recapitulation of the setup inherited
...
and the manner of its failure, which turns out to be diagnostic rather than merely
discouraging: what gave way was not the ingenuity of the search but the assumption
that there was a formula to be found. The explanation is dynamical. Under the
...
A word on why the scope section exists. There are at least four inequivalent things
one might mean by ``$A(p)$ has no closed form'', and a theorem about one of them is
not a theorem about another. What is proved here concerns the conjugacy as a
function of its own dynamical variable. It does not decide the arithmetic nature of
the individual numbers $A(p_0)$, nor whether the map $p\mapsto A(p)$ is elementary.
Both remain open, and the chapter says so wherever the distinction could otherwise
be blurred.
