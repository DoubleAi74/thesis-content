...
recording, and so is the manner of the failure, because the manner of the failure
is itself evidence.

The first method was the continuum route of \cref{a:sec:nearcritical}: treat the map
...
is to say it leaves $A(p)$ undetermined. That is not an accident of the method. The
constant of integration is fixed by the early generations, where the continuum
approximation is invalid, and $A(p)$ is exactly the accumulated record of those
generations.
...
\end{equation*}
rather than the required $-4$. Matching the endpoint values while missing the slope
is enough to make the relative error in $A$, and hence the error in $1/A$,
unacceptable near criticality.
...
exercise, not proposed identities, and they are reproduced here only to show what
the search returned.
...
very different formulae can behave almost identically over a bounded interval is not
...
quantity of fitting will ever settle the question. Something structural is needed,
and there is something structural available.
