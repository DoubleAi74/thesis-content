% --- Chapter appendices: numbered M.A, M.B ----------------------------------
...
along the interval of integration.
\end{proposition}
...
$\Gamma(z+1) = z\Gamma(z)$,
\begin{equation*}
\int_0^1\frac{u^{h/k-1}}{1+\frac{Q}{P}u}\,\dd u
...
\end{equation*}
and restoring $v = \ee^{k\tau}$ gives \cref{m:eq:hypIden} on the Euler domain.
Both sides are analytic antiderivatives away from the singular set, so continuation
along any admissible interval preserves the identity.
\end{proof}
...
$Q = -(\sigma-x_+)$ --- \cref{m:prop:hypIden} reproduces \cref{m:eq:Psihyp} once
the prefactors $\alpha x_+(\sigma-x_-)$ and $-\alpha x_-(\sigma-x_+)$ are carried
...
catches it immediately. At the resonant values $\alpha = n(\mu-\lambda)$ of
\cref{m:rem:resonance} the same conclusion follows by the finite parameter limit
described there.
\end{remark}
...
\int_{|x|=\varrho_x}\int_{|y|=\varrho_y}
...
\cref{m:eq:absdeathstates}, with the continuous equal-rate limit of
\cref{m:eq:equalrates} when $\alpha=\mu$, so that
\begin{equation*}
p_{i,j}(t) = \frac{n!}{i!\,j!\,(n-i-j)!}\;f(t)^{i}g(t)^{j}c(t)^{\,n-i-j} .
\end{equation*}
Each particle independently ends up dead, absorbed, or still outside, with those
three probabilities, and the joint law is trinomial; since $c+f+g=1$, summing over
$i$ and $j$ recovers one by the multinomial theorem. That the generating function
factorises in this way is \cref{m:eq:indRelPGF} read backwards.

\subsection*{The absorption--birth--death case}

The generating function of \cref{m:prop:Gabd} is not of the multilinear form
\cref{m:eq:multilinear}, but its dependence on $y$ remains linear before the power
$N$ is taken. Writing $g(x,y,t) = \ee^{-\alpha t}y + h(x,t)$ with
\begin{equation*}
h(x,t) = \alpha\ee^{-\alpha t}\int_0^t\ee^{\alpha v}F(x,v)\,\dd v
\end{equation*}
as in \cref{m:sec:regularrep}, the binomial theorem gives
\begin{equation*}
G(x,y,t) = \sum_{j=0}^{N}\binom{N}{j}\ee^{-\alpha tj}y^j\,h(x,t)^{N-j},
\end{equation*}
and therefore
\begin{equation}
\label{m:eq:abdcoeffapp}
p_{i,j}(t) = \binom{N}{j}\ee^{-\alpha tj}\,[x^i]h(x,t)^{N-j} .
\end{equation}
If $h(x,t)=\sum_{k\ge0}h_k(t)x^k$, the remaining coefficient is the finite
convolution
\begin{equation*}
[x^i]h(x,t)^M
= \sum_{\substack{k_1+\cdots+k_M=i\\k_r\geq0}}\prod_{r=1}^{M}h_{k_r}(t) ,
\end{equation*}
in which truncating $h$ after degree $i$ is exact: coefficients of higher degree
cannot contribute to $[x^i]$. Alternatively \cref{m:eq:stateProb} or
\cref{m:eq:cauchy2d} may be applied to \cref{m:eq:Gabd} directly. As a check,
summing over all interior states at fixed $j$ means setting $x=1$; since
$h(1,t) = 1-\ee^{-\alpha t}$,
\begin{equation*}
