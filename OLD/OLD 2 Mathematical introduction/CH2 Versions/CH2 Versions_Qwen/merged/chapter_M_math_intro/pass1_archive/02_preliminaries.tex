...
functions may go straight to \cref{m:sec:gw}. It is included because three things are
...
For several counts tracked at once the multivariate extension is
\begin{equation}
\label{m:eq:pgfmulti}
\phi_{\mathbf X}(z_1,\dots,z_d)
= \ex{z_1^{X_1}\cdots z_d^{X_d}}
= \sum_{k_1,\dots,k_d\ge0}\pr{X_1=k_1,\dots,X_d=k_d}\,z_1^{k_1}\cdots z_d^{k_d},
\end{equation}
with each marginal recovered by setting the remaining variables to one. Multi-type
generating functions of this kind are the natural language of the two-type
extensions treated \fwd{multitype}{in the chapter on multi-type processes}; no more
is needed of them here.
...
$\lambda>\mu$. \Cref{m:sec:cts} takes \cref{m:eq:p0t} as its starting point.

\subsection{First-step analysis}
\label{m:sec:firststep}

One last instrument is used repeatedly below, in both discrete and continuous
time. When the question is not the full time course but a hitting probability or a
mean time, one conditions on the first exit from the current state and solves the
resulting self-consistent equation. The gesture is the same each time: one step of
randomness, then the Markov property restarts the problem.

In discrete time the extinction probability of \cref{m:sec:gw} is the prototype:
conditioning on the fate of the founder gives $u_{n+1}=\phi(u_n)$, and the ultimate
extinction probability is a fixed point of $\phi$. In continuous time the same
conditioning is applied to the winner of the exponential race of
\cref{m:prop:race}: the backward equation of \cref{m:sec:cts} is obtained by asking
what happens in a first interval of length $h$ and letting $h\downarrow0$. Nothing
deeper is required of the method; it is recorded here only so that later uses can
be recognised as instances of it.
