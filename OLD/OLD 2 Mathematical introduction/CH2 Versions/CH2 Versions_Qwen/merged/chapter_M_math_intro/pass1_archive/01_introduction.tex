...
$A(p)$ and derives what quasi-stationarity needs from it. The constant itself ---
what it is, how to compute it, and why no elementary formula for it has been found
despite a determined search --- is the subject of \ChA, which develops its product
and series representations, its bounds and near-critical asymptotics, and the
dynamical obstruction to any closed form.
...
introduces $A(p)$. \Cref{m:sec:small} asks what changes when the process starts from a
...
the generating-function equations of absorption models. The analysis of the
discrete constant $A(p)$ beyond its definition and first moments is reserved for
\ChA.

Two remarks on how the chapter is written. First, the closed form obtained in
\cref{m:sec:absbirthdeath} for the absorption--birth--death generating function is
a result of this thesis, placed here because it is produced by the machinery the
section develops and because later chapters use it throughout. Everything else in
the chapter is standard, and is presented as such; the deep properties of $A(p)$
quoted from \ChA are proved there. Second, the argument is in places heuristic ---
