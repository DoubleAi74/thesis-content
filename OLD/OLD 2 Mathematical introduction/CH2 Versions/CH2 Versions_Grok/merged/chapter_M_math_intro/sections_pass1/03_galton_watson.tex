...
\end{equation}
as \cref{m:fig:power_law}(a) confirms. Substituting into \cref{m:eq:ETsumSn}, the tail
...
\label{m:sec:total-progeny}

Write $T=\sum_{n\ge0}Z_n$ for the total progeny of a critical binary Galton--Watson
process ($p=\tfrac12$), started from $Z_0=1$. Conditioning on the first generation
gives the functional equation $g(s)=s\,\phi(g(s))$ for $g(s)=\ex{s^T}$ with
$\phi(z)=\tfrac12+\tfrac12 z^2$, hence
\begin{equation}
\label{m:eq:total-progeny-gf}
g(s) = \frac{1-\sqrt{1-s^2}}{s}\qquad(s\in(0,1]),
\end{equation}
and the coefficients are Catalan:
\begin{equation}
\label{m:eq:catalan}
\pr{T=2n+1} = \frac{1}{n+1}\binom{2n}{n}\frac{1}{2^{2n}},
\qquad n\ge0.
\end{equation}
In particular $\ex{T}=\infty$, consistent with infinite expected lifetime at
criticality (\cref{m:sec:criticalcase}).

\FloatBarrier

\begin{remark}[What is elementary and what is not]
\label{m:rem:elem-vs-not}
The critical lifetime asymptotics of this section are elementary, as is the
continuous-time Kolmogorov constant $\Ac(p)$ developed in \cref{m:sec:cts}. The
discrete constant $A(p)$ that governs the subcritical conditional mean is not known
to admit an elementary closed form; \ChA develops its product and series
representations and the dynamical obstruction that explains the difficulty.
\end{remark}
