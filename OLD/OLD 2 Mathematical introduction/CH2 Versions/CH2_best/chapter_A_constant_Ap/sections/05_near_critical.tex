...
settles it, and does so more transparently than the matching argument one is first
tempted to try. It also identifies the order of the first relative correction,
...
which the geometric factor $r^n$ still carries weight. The heuristic version of the
same computation is worth seeing, because it makes the mechanism visible. The
...
\end{equation}
This is the single most useful diagnostic in \cref{a:sec:GA}. A candidate closed
form may reproduce $A$ to plotting accuracy across the whole interval and still fail
here, and failing here is fatal, because as $A\to0$ an error in $A$ of vanishing
absolute size produces an unbounded error in the quasi-stationary mean $1/A$.
\end{remark}
...
\qquad c\approx0.78,
\end{equation}
consistent with \cref{a:eq:Anearcritical}. The constant $c$ is empirical and is not
used anywhere below; the proved statement is the order estimate. What the
logarithmic correction does control is where the leading asymptotic becomes an
acceptable substitute for the exact value, and hence the crossover chosen in
...
\ChM, and it is worth seeing why that route is inferior. The exact difference is
\begin{equation*}
...
the error, and so cannot see the logarithm of \cref{a:eq:Anearcritical}. The series
argument needs no matching and delivers the remainder.
