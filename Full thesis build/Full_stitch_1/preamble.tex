...
% Drop appendix headings from the contents list; they remain in the chapter body.
% (hyperref still writes \contentsline when tocdepth is lowered, so we
%  silence addcontentsline for the toc file instead.)
\makeatletter
\newcommand{\omitfromtoc}{%
  \global\let\savedaddcontentsline\addcontentsline
  \gdef\addcontentsline##1##2##3{%
    \def\stitch@tocarg{##1}%
    \def\stitch@tocname{toc}%
    \ifx\stitch@tocarg\stitch@tocname
    \else
      \savedaddcontentsline{##1}{##2}{##3}%
    \fi
  }%
}
\newcommand{\restoretoc}{%
  \global\let\addcontentsline\savedaddcontentsline
  \setcounter{tocdepth}{2}%
}
\makeatother
\allowdisplaybreaks
...
% Sentence-initial form of \mextref.
\providecommand{\Mextref}[2]{%
  \ifcsname r@#1\endcsname\Cref{#1}\else#2\fi}
