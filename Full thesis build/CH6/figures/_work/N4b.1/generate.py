...
def make_figure(plot_data: tuple[np.ndarray, ...]) -> plt.Figure:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(5.89, 2.45))
    ...
    ax_schedule.plot(
        ...
        linewidth=2.1,
        linestyle="--",
        label=r"constant $p\,\mathrm{e}^{-d_{\mathcal{I}}\alpha}$",
    )
    ...
    ax_schedule.set(
        xlim=(0.0, 12.0),
        ylim=(0.0, 5.0),
        xlabel=r"cell age $\alpha$",
        ylabel=r"expected release flux",
    )
    ax_schedule.legend(loc="upper right")
    style_rc.panel_label(ax_schedule, "(a)")

    ax_response.axvspan(0.0, 1.5, color=style_rc.SOFT, alpha=0.09, zorder=0)
    ax_response.plot(
        age,
        kernel_response,
        color=style_rc.NAVY,
    ...
    ax_response.annotate(
        ...
        xytext=(3.15, 0.72),
        arrowprops=dict(
            arrowstyle="->", color=style_rc.SOFT, linewidth=1.0
        ),
        ha="left",
        va="center",
        fontsize=8.5,
    ...
    ax_response.set(
        ...
        ylim=(0.0, 3.35),
        xlabel=r"time $t$",
        ylabel="free particles per initial cell",
    )
    ax_response.legend(loc="upper right")
    style_rc.panel_label(ax_response, "(b)")
