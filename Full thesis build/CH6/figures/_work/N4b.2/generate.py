...
def make_figure(family: list[Triple]) -> None:
    ...
    fig, (ax_slice, ax_family) = plt.subplots(1, 2, figsize=(5.89, 2.8))
    ...
    style_rc.panel_label(ax_slice, "(a)")
    ...
    for index, label, offset in (
        (12, "D", (14, 2)),
        (reference_index, "reference", (16, 6)),
        (-8, "F", (-8, 12)),
    ):
        point = family[index]
        if label == "reference":
    ...
    ax_family.text(
        1.0,
        1.03,
        r"$R_0=2$ and $r=0.294$ throughout;"
    ...
    style_rc.panel_label(ax_family, "(b)")
