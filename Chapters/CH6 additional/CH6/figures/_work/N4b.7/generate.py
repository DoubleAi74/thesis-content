...
def make_figure(checked: list[dict[str, float]]) -> None:
    ...
    for column, (regime, values) in enumerate(zip(REGIMES, checked)):
        ...
        ax_release.set_xlabel(r"cell age $\alpha$")
        style_rc.panel_label(ax_survival, f"({top_letters[column]})")
        style_rc.panel_label(ax_release, f"({bottom_letters[column]})")

    axes[0, 0].set_ylabel(r"survival $I_{\rm fix}(\alpha)$")
    axes[1, 0].set_ylabel(r"release $g(\alpha)$")
    for ax in axes.flat:
        ax.set_xlim(0.0, 12.0)
        ax.set_xticks(np.arange(0.0, 12.1, 2.0))

    fig.tight_layout(w_pad=0.9, h_pad=1.1, rect=(0.0, 0.0, 1.0, 0.97))
